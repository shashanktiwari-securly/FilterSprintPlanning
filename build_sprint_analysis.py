"""
Build a FILTER sprint analysis workbook (Cases-style) for ANY sprint.

Tabs (in order):
    1. Capacity vs Delivery        — capacity, delivery, completed-by-priority,
                                     sprint completion, snapshots
    2. Per-person Delivery
    3. Completed
    4. Not Completed
    5. Removed
    6. P1 Escalation               — Escape Defect + Priority P1 + 'P1 Escalation'
                                     in the title

Inputs (regenerate with these scripts):

* ``product_FILTER/<slug>-sprint-data/sprint-meta.json``
    ``python fetch_filter_sprint_meta.py <sprint name>
        --out product_FILTER/<slug>-sprint-data/sprint-meta.json``

* ``product_FILTER/<slug>-sprint-data/issues.json``
    ``python fetch_filter_sprint_issues.py <sprint name>``
    (now scoped to FILTER + PTGM + FDSE — see PROJECT_KEYS_IN_SCOPE.)

Per-person availability comes from ``compute_capacity.py`` and uses:
    * Company holiday calendar (India 2026)
    * Date-aware roster — Amol Mithari joined 2026-02-20; Rachit Mishra's last
      working day was 2026-02-20

Output: ``FILTER Sprint <Name> Analysis.xlsx`` at the repo root.

Usage::

    python build_sprint_analysis.py sprint-Arthur
    python build_sprint_analysis.py sprint-Bertha --out "FILTER Sprint Bertha Analysis.xlsx"
"""
...
import compute_capacity as cap  # noqa: E402
...
INACTIVE_ROSTER_FILL = PatternFill("solid", fgColor="FFE7E6E6")
...
def per_person_totals(
    records: list[dict], field: str, names: list[str]
) -> dict[str, float]:
...
def priority_breakdown(
    ...
    for rec in records:
        ...
        bucket["count_all"] += 1
        bucket["hours"] = round(
            bucket["hours"] + float(rec.get("time_spent_hrs") or 0.0), 2
        )
    for v in out.values():
        v.pop("_keys", None)
...
def write_p1_escalation_sheet(ws, records: list[dict], sprint_meta: dict) -> int:
    ...
    ws.merge_cells(
        start_row=1, start_column=1, end_row=1, end_column=len(DETAIL_HEADERS)
    )

    subtitle = (
        f"{sprint_meta['sprint_name']} tickets where Issue Type = Escape Defect AND "
        f"Priority = P1 AND summary contains 'P1 Escalation' (regardless of bucket). "
        f"Total = {len(records)}."
    )
    ws.cell(2, 1, subtitle).font = Font(italic=True)
    ws.merge_cells(
        start_row=2, start_column=1, end_row=2, end_column=len(DETAIL_HEADERS)
    )
    ...
    if records_sorted:
        ws.auto_filter.ref = (
            f"A{header_row}:{get_column_letter(len(headers_full))}"
            f"{header_row + len(records_sorted)}"
        )
    return len(records_sorted)


def build_team_composition_for_sprint(
    sprint_start: date, sprint_end: date
) -> tuple[list[tuple], list[str]]:
    """Return (composition_rows, active_team_names) honoring the date-aware roster.
    Inactive members are kept in the table with FTE 0 and a note explaining why."""
    rows: list[tuple] = []
    active_names: list[str] = []
    for name, role, func, fte, seniority, employment, note in TEAM_COMPOSITION_BASE:
        ...
        if bounds:
            ...
            if start_d and sprint_end < start_d:
                ...
                new_fte = 0.0
                new_note = (
                    f"Joined team on {start_d:%d-%b-%Y} — not part of this sprint"
                )
            elif end_d and sprint_start > end_d:
                active = False
                new_fte = 0.0
                new_note = (
                    f"Left team on {end_d:%d-%b-%Y} — not part of this sprint"
                )
            elif start_d and sprint_start < start_d <= sprint_end:
                new_note = (
                    (note + " · " if note else "")
                    + f"Joined {start_d:%d-%b-%Y} (partial sprint)"
                )
            elif end_d and sprint_start <= end_d < sprint_end:
                new_note = (
                    (note + " · " if note else "")
                    + f"Last day {end_d:%d-%b-%Y} (partial sprint)"
                )
        rows.append((name, role, func, new_fte, seniority, employment, new_note))
        if active and name in CORE_TEAM_ORDER:
            active_names.append(name)
    return rows, active_names


def slugify_filename(name: str) -> str:
    """sprint-Arthur → 'Arthur'; sprint-A_B → 'A_B'."""
...
def main() -> None:
    ...
    ap.add_argument("sprint_name", help="Jira sprint name, e.g. sprint-Arthur")
    ap.add_argument(
        "--folder",
        default=None,
        help="Path to <slug>-sprint-data folder (default: product_FILTER/<sprint>-sprint-data)",
    )
    ap.add_argument(
        "--out",
        default=None,
        help="Output workbook path (default: 'FILTER Sprint <Name> Analysis.xlsx' at repo root)",
    )
    args = ap.parse_args()
    ...
    if args.folder:
        ...
        folder = ROOT / "product_FILTER" / f"{sprint_name}-sprint-data"
    if not folder.is_dir():
        raise SystemExit(f"Sprint folder not found: {folder}")

    issues_file = folder / "issues.json"
    if not issues_file.is_file():
        raise SystemExit(
            f"Missing {issues_file}. Run from repo root:\n"
            f"  python fetch_filter_sprint_issues.py {sprint_name}"
        )
    ...
    holiday_descr = (
        ", ".join(f"{d:%d-%b}: {name}" for d, name in company_holidays)
        or "none in window"
    )
    eff_sprint_wd = cap.effective_working_days_in_sprint(sprint_start, sprint_end)
    composition_rows, active_team_names = build_team_composition_for_sprint(
        sprint_start, sprint_end
    )

    def in_core(name: str | None) -> bool:
        return bool(name) and name in active_team_names

    by_key = {r["key"]: r for r in issues}
    completed_recs = [
        by_key[k]
        for k in bucket["completed"]
        if k in by_key and in_core(by_key[k].get("assignee"))
    ]
    not_completed_recs = [
        by_key[k]
        for k in bucket["not_completed"]
        if k in by_key and in_core(by_key[k].get("assignee"))
    ]
    removed_recs = [
        by_key[k]
        for k in bucket["removed"]
        if k in by_key and in_core(by_key[k].get("assignee"))
    ]

    completed_hrs_by = per_person_totals(
        completed_recs, "time_spent_hrs", active_team_names
    )
    notcomp_hrs_by = per_person_totals(
        not_completed_recs, "original_estimate_hrs", active_team_names
    )
    removed_hrs_by = per_person_totals(
        removed_recs, "original_estimate_hrs", active_team_names
    )
    ...
    def _sum_oe(records: list[dict]) -> float:
        return round(
            sum(float(r.get("original_estimate_hrs") or 0.0) for r in records), 2
        )
    ...
    cap_ws.cell(
        2, 1,
        "Team Composition (per Securly-PjM-Skills.md Part 2 — Filter India roster). "
        "Inactive members for this sprint window are shaded grey with a roster note.",
    ).font = Font(bold=True)
    ...
    for c, h in enumerate(comp_headers, 1):
        ...
        cell.fill = HEADER_FILL
    row_index = 4
    for member in composition_rows:
        for c, val in enumerate(member, 1):
            cell = cap_ws.cell(row_index, c, val)
            if c == 4:
                cell.number_format = "0.0"
        if member[3] == 0.0 and member[0] != "Shashank Tiwari":
            for c in range(1, 8):
                cap_ws.cell(row_index, c).fill = INACTIVE_ROSTER_FILL
        row_index += 1
    total_alloc_row = row_index
    cap_ws.cell(total_alloc_row, 1, "Total FILTER allocation").font = Font(bold=True)
    cap_ws.cell(
        total_alloc_row, 4, f"=SUM(D4:D{total_alloc_row-1})"
    ).number_format = "0.0"
    ...
    cap_ws.cell(
        row_index, 1,
        "Capacity Parameters (yellow cells are inputs — re-run the script after edits)",
    ).font = Font(bold=True)
    ...
    cap_ws.cell(
        row_index, 3,
        f"FILTER 15-day sprint Tue → Mon. {sprint_name} = {sprint_start} → {sprint_end}.",
    )
    row_index += 1
    ...
    cap_ws.cell(
        row_index, 2, f"=B{p_rows['wd']}-B{p_rows['holidays']}"
    ).number_format = "0"
    ...
    cap_ws.cell(
        row_index, 3,
        f"{int(MISC_BUFFER_PCT*100)}% blanket reservation deducted from Available "
        "to get Net Available",
    )
    row_index += 1
    ...
    cap_ws.cell(
        row_index, 2,
        f"=B{p_rows['effwd']}*B{p_rows['hpd']}*B{p_rows['focus']}",
    ).number_format = "0.00"
    cap_ws.cell(row_index, 3, "= effective days × hrs/day × focus factor")
    row_index += 1
    p_rows["fte"] = row_index
    cap_ws.cell(row_index, 1, "Allocated FTE on FILTER (active this sprint)")
    c = cap_ws.cell(row_index, 2, len(active_team_names)); c.fill = INPUT_FILL; c.number_format = "0"
    cap_ws.cell(
        row_index, 3,
        "Headcount actually working this sprint (excludes joiners/leavers outside window)",
    )
    row_index += 1
    p_rows["ideal"] = row_index
    cap_ws.cell(row_index, 1, "Ideal capacity for the sprint (hrs)")
    cap_ws.cell(
        row_index, 2, f"=B{p_rows['eff']}*B{p_rows['fte']}"
    ).number_format = "0.00"
    cap_ws.cell(row_index, 3, "= effective hrs/FTE × allocated FTE")
    row_index += 1
    p_rows["scope"] = row_index
    cap_ws.cell(row_index, 1, "Issue scope (Jira projects)")
    cap_ws.cell(
        row_index, 2, ", ".join(PROJECT_KEYS_IN_SCOPE)
    )
    cap_ws.cell(
        row_index, 3,
        "Tickets pulled with `project in (FILTER, PTGM, FDSE) AND sprint = …` — "
        + " · ".join(f"{k}: {v}" for k, v in sorted(project_dist.items())),
    )

    row_index += 2
    cap_ws.cell(
        row_index, 1, f"Capacity vs Actual Delivery — {sprint_name}"
    ).font = Font(bold=True)
    cap_ws.merge_cells(start_row=row_index, start_column=1, end_row=row_index, end_column=7)
    row_index += 1
    for c, h in enumerate(
        ["Metric", "Hours", "% of Ideal", "Rating", "Notes"], 1
    ):
        cell = cap_ws.cell(row_index, c, h)
        cell.font = HEADER_FONT
        cell.fill = HEADER_FILL
    ...
    metrics = [
        ("Actual Availability",
         f"='Per-person Delivery'!F{pp_total_row_placeholder}", None,
    ...
    for label, hrs_formula, pct_formula, notes in metrics:
        ...
        if pct_formula is not None:
            cap_ws.cell(
                row_index, 3,
                pct_formula.format(row=row_index, net_row=net_row),
            ).number_format = "0.0%"
    ...
    cap_ws.cell(
        row_index, 1,
        "Tickets (parents) = unique parent items (Story / Task / Defect / Escape Defect) — "
        "subtasks roll up to their parent so 1 parent + 3 subtasks = 1 ticket. "
        "Tickets (incl. subtasks) = raw record count incl. every Dev / QA / Automation / "
        "Review / Test Case Creation / Debug subtask. "
        "Hours = sum across parents and subtasks; matches the core-team Completed hours on "
        "the Per-person Delivery sheet.",
    ...
    total_completed_tickets_all = sum(
        v["count_all"] for v in completed_priority.values()
    )
    ...
    for p in PRIORITY_ORDER:
        ...
        if p == "P0":
            ...
            cell = cap_ws.cell(
                row_index, 7,
                "P1 — high priority (see P1 Escalation tab for escalations)",
            )
    ...
    cap_ws.cell(
        row_index, 1, "Sprint Completion (spillover = Not Completed)"
    ).font = Font(bold=True)
    cap_ws.merge_cells(start_row=row_index, start_column=1, end_row=row_index, end_column=7)
    row_index += 1
    sc_headers = [
        "Metric", "Tickets", "Planned Hrs (OE)", "% by Tickets", "% by Hrs", "Notes",
    ]
    ...
    sprint_completion_count = (
        n_completed / in_sprint_tickets if in_sprint_tickets else 0
    )
    sprint_completion_hrs = (
        completed_oe_hrs / in_sprint_oe if in_sprint_oe else 0
    )
    plan_completion_count = (
        n_planned_completed / planned_in_sprint_tickets
        if planned_in_sprint_tickets else 0
    )
    plan_completion_hrs = (
        planned_completed_oe / planned_in_sprint_oe
        if planned_in_sprint_oe else 0
    )
    ...
    summary_rows = [
        ...
        ("Planned completion rate (tickets, excludes mid-sprint adds)",
         plan_completion_count, "Originally-committed tickets only"),
        ("Planned completion rate (planned hrs, excludes mid-sprint adds)",
         plan_completion_hrs, "Originally-committed planned hours only"),
    ]
    ...
    p1_open = sum(
        1 for r in p1_escalation_recs if (r.get("status_category") or "") != "done"
    )
    ...
    not_completed_added = sum(
        1 for r in not_completed_recs if r.get("added_during_sprint")
    )
    removed_added = sum(1 for r in removed_recs if r.get("added_during_sprint"))
    cap_ws.cell(
        row_index, 1, "Mid-sprint additions snapshot"
    ).font = Font(bold=True, color="FF7F6000", size=12)
    cap_ws.merge_cells(start_row=row_index, start_column=1, end_row=row_index, end_column=7)
    row_index += 1
    add_summary = (
    ...
    row_index += 1
    notes_lines = [
        f"• Sprint window from Jira "
        f"({sprint_meta.get('api_self', '/rest/agile/1.0/sprint/' + str(sprint_meta['sprint_id']))}): "
        ...
        "• Issues pulled from FILTER + PTGM + FDSE (project-scope expanded for the India "
        "scrum team's split across the three products).",
        "• Date-aware roster: Amol Mithari joined 20-Feb-2026; Rachit Mishra's last day was "
        "20-Feb-2026. Inactive members for this sprint window are shaded grey in the Team "
        "Composition table and excluded from per-person availability + delivery.",
        "• Available (hrs) = (effective working days − leave days) × 8 × focus factor, where "
        "effective working days = sprint working days − company holidays falling inside the "
        "sprint − days outside the person's employment window. Per-person Delivery shows "
        "leave days in Column G to explain the delta from a full sprint.",
        f"• Net Available = Available − {int(MISC_BUFFER_PCT*100)}% Misc Buffer for meetings, "
        "scrums, code review, etc. Completed/Not Completed/Removed % of capacity use Net "
        ...
        "• Core roster only: assignees outside Securly-PjM-Skills.md Part 2 are dropped from "
        "per-person + detail sheets.",
        "• P1 Escalation tab is filtered to Issue Type = Escape Defect AND Priority = P1 AND "
        "title contains 'P1 Escalation'.",
        "• Completed by Priority — Tickets (parents) rolls subtasks up to their parent (each "
        "work item counted once at its parent priority); Tickets (incl. subtasks) shows the "
        "raw record count for comparison. The Hours column sums every record (parent + "
        "subtasks) and matches the core-team Completed total.",
        "• Spillover = Not Completed bucket (tickets carried into the next sprint).",
        "• Sprint completion rate uses Completed ÷ (Completed + Spillover); Removed tickets "
        "are excluded (treated as scope cut). 'Planned' rate also excludes mid-sprint "
        "additions.",
        "• All detail sheets (Completed, Not Completed, Removed) highlight tickets added "
        "after sprint start (Jira sprint-report issueKeysAddedDuringSprint) in amber, with "
        "an 'Added Mid-Sprint = YES' tag column.",
        "• Ratings: ≥85% On Target (green), 70–85% Below Target (amber), <70% Significantly "
        "Below (red).",
    ]
    ...
    pp_headers = [
        ...
        "Available (hrs)",
        "Leave Days (sprint)",
        f"Misc Buffer {int(MISC_BUFFER_PCT*100)}% (hrs)",
        "Net Available (hrs)",
    ]
    ...
    for i, name in enumerate(active_team_names, 2):
        pp_ws.cell(i, 1, name)
    ...
    if args.out:
        ...
        out = ROOT / f"FILTER Sprint {slugify_filename(sprint_name)} Analysis.xlsx"
    try:
        wb.save(out)
        ...
        wb.save(out)
    print(f"Wrote {out}")
    excluded_names = [n for n in CORE_TEAM_ORDER if n not in active_team_names]
    roster_note = (
        ", ".join(excluded_names) + " excluded" if excluded_names else "all active"
    )
    print(
        f"  active_team={len(active_team_names)} of {len(CORE_TEAM_ORDER)} ({roster_note})"
    )
    print(
    ...
    print(
        f"  total Completed (in-sprint hrs):     {sum(completed_hrs_by.values()):.2f}"
    )
    print(
        f"  total Not Completed (orig est hrs):  {sum(notcomp_hrs_by.values()):.2f}"
    )
    print(
        f"  total Removed (orig est hrs):        {sum(removed_hrs_by.values()):.2f}"
    )
    print(
        f"  Actual Availability (sum):           {sum(avail_hours.values()):.2f}"
    )
    print(
    ...
    print(
        f"  Project distribution: " + ", ".join(
            f"{k}={v}" for k, v in sorted(project_dist.items())
        )
    )
    ...
    print(
        f"  Sprint completion (tickets): {n_completed}/{in_sprint_tickets} = "
        f"{sprint_completion_count*100:.1f}%   "
        f"(planned: {n_planned_completed}/{planned_in_sprint_tickets} = "
        f"{plan_completion_count*100:.1f}%)"
    )
    print(
        f"  Sprint completion (planned hrs): {completed_oe_hrs:.2f} / {in_sprint_oe:.2f} "
    ...
    print(
        ...
        f"(Completed {completed_added}, Spillover {not_completed_added}, "
        f"Removed {removed_added})"
    )
